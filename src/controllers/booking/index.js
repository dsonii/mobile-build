...
module.exports = {
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      let pick_lang = '';
      let pick_address = '';
      if (typeof passengerDetailsItem[0] != 'undefined' ){
        pick_lat = passengerDetailsItem[0].pick_lat;
        pick_lang = passengerDetailsItem[0].pick_lang;
        pick_address = passengerDetailsItem[0].pick_address;
      }
      // Start session
      await session.startTransaction();
      ...
      if (offer_code.trim() != "") {
        ...
        if (typeof fareData.created_date == 'undefined' || fareData.created_date == 'Invalid date') {
          ...
          fareData.created_date = moment(newDate)
              .tz("Asia/Kolkata")
              .format("YYYY-MM-DD");
        }
        saveObj = {
    ...
    try {
      ...
      if (
        await Payment.exists({
          ...
          let findPayment = await Payment.findOne({ orderId: orderId});
          if (findPayment.payment_mode == 'cash') {
            if (await Payment.exists({ orderId, payment_status: "Processing" })) {
              if (status === "true") {
                ...
                let bookingId = findPayment.bookingId
                const getBooking = await Booking.findOne(bookingId);
                const pnr_no = getBooking.pnr_no;
                const updateObj = {
                  travel_status: "SCHEDULED",
                };
                const updateBooking = await Booking.findOneAndUpdate(
